%Ejercicio 2.2:
%Función de Rosenbrock
%Hay que aplicar los 3 métodos a la función para n ∈ {2, 8, 32, 128} . Para cada método haga una
%tabla que contiene las últimas 5 iteraciones con su número (para cada n)

%1. Definimos los parámetros dados, recordando que la tol se define dentro de
%las funciones. 

c1 = 1e-4;
c2 = 0.99;
tol = 1e-5;
eta = 0.1;
DeltaMax = 1.5;
itmax = 10000;
nvalues = [2;8;32;128];
m = length(nvalues);

f = @(x) frosenbrock(x);
    
fprintf("------------Resultados del Problema Rosenbrock------------\n")


%% Para n=2
n = 2;
x0 = ones(n,1);
%Generamos X0
for i=1:n
       if mod(i,2)~=0
           x0(i)=-1.2;
       end
end
%Buscamos el numero de iteraciones para tomar las 5 últimas iteraciones
[xk, it_SR1] = rcSCR1(f, x0,itmax);
[xk, it_BFGS] = lsBFGS( f, x0, itmax);
[xk, it_Mem] = lineBGFS( f, x0,itmax );

% Buscamos los valores ||∇f(xk)|| , f(xk), los errores ||xk − x*|| y el
% tiempo para las cinco últimas iteraciones. 

%Para rcSCR1
%vectores auxiliar para las iteraciones
ng_SCR1 = zeros(5,1); 
f_SCR1 = zeros(5,1);
e_SCR1 = zeros(5,1);
t_SCR1 = zeros(5,1);
aux = zeros(5,1);
xaux = ones(n,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_SR1-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = rcSCR1(f,x0,aux(k));
    time = toc;
    ng_SCR1(k) = norm(apGrad(f,xk));
    f_SCR1(k) = f(xk);
    e_SCR1(k) = norm(xk-xaux);
    t_SCR1(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con SCR y n=2 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_SCR1(j), f_SCR1(j), e_SCR1(j), t_SCR1(j))
end

%Para lsBFGS
%vectores auxiliar para las iteraciones
ng_BFGS = zeros(5,1); 
f_BFGS = zeros(5,1);
e_BFGS = zeros(5,1);
t_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(2,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_BFGS-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lsBFGS(f,x0,aux(k));
    time = toc;
    ng_BFGS(k) = norm(apGrad(f,xk));
    f_BFGS(k) = f(xk);
    e_BFGS(k) = norm(xk-xaux);
    t_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con lsBFGS y n=2 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_BFGS(j), f_BFGS(j), e_BFGS(j), t_BFGS(j))
end
%Para BFGS Limited Memory
%vectores auxiliar para las iteraciones
ngmem_BFGS = zeros(5,1); 
fmem_BFGS = zeros(5,1);
emem_BFGS = zeros(5,1);
tmem_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(2,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_Mem-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lineLM_BFGS(f,x0,aux(k),itmax,m);
    time = toc;
    ngmem_BFGS(k) = norm(apGrad(f,xk));
    fmem_BFGS(k) = f(xk);
    emem_BFGS(k) = norm(xk-xaux);
    tmem_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con LMBFGS y n=2 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ngmem_BFGS(j), fmem_BFGS(j), emem_BFGS(j), tmem_BFGS(j))
end


%% Para n=8
n = 8;
x0 = ones(n,1);
%Generamos X0
for i=1:n
       if mod(i,2)~=0
           x0(i)=-1.2;
       end
end
%Buscamos el numero de iteraciones para tomar las 5 últimas iteraciones
[xk, it_SR1] = rcSCR1(f, x0,itmax);
[xk, it_BFGS] = lsBFGS( f, x0, itmax);
[xk, it_Mem] = lineBGFS( f, x0,itmax );

% Buscamos los valores ||∇f(xk)|| , f(xk), los errores ||xk − x*|| y el
% tiempo para las cinco últimas iteraciones. 

%Para rcSCR1
%vectores auxiliar para las iteraciones
ng_SCR1 = zeros(5,1); 
f_SCR1 = zeros(5,1);
e_SCR1 = zeros(5,1);
t_SCR1 = zeros(5,1);
aux = zeros(5,1);
xaux = ones(n,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_SR1-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = rcSCR1(f,x0,aux(k));
    time = toc;
    ng_SCR1(k) = norm(apGrad(f,xk));
    f_SCR1(k) = f(xk);
    e_SCR1(k) = norm(xk-xaux);
    t_SCR1(k) = time;
    k=k+1;
end



%Construimos tabla:
fprintf("------------Resultados con SCR y n=8 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_SCR1(j), f_SCR1(j), e_SCR1(j), t_SCR1(j))
end

%Para lsBFGS
%vectores auxiliar para las iteraciones
ng_BFGS = zeros(5,1); 
f_BFGS = zeros(5,1);
e_BFGS = zeros(5,1);
t_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(n,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_BFGS-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lsBFGS(f,x0,aux(k));
    time = toc;
    ng_BFGS(k) = norm(apGrad(f,xk));
    f_BFGS(k) = f(xk);
    e_BFGS(k) = norm(xk-xaux);
    t_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con lsBFGS y n=8 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_BFGS(j), f_BFGS(j), e_BFGS(j), t_BFGS(j))
end

%Para BFGS Limited Memory
%vectores auxiliar para las iteraciones
ngmem_BFGS = zeros(5,1); 
fmem_BFGS = zeros(5,1);
emem_BFGS = zeros(5,1);
tmem_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(2,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_Mem-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lineLM_BFGS(f,x0,aux(k),m);
    time = toc;
    ngmem_BFGS(k) = norm(apGrad(f,xk));
    fmem_BFGS(k) = f(xk);
    emem_BFGS(k) = norm(xk-xaux);
    tmem_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con LMBFGS y n=8 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ngmem_BFGS(j), fmem_BFGS(j), emem_BFGS(j), tmem_BFGS(j))
end

%% Para n=32
n = 32;
x0 = ones(n,1);
%Generamos X0
for i=1:n
       if mod(i,2)~=0
           x0(i)=-1.2;
       end
end
%Buscamos el numero de iteraciones para tomar las 5 últimas iteraciones
[xk, it_SR1] = rcSCR1(f, x0,itmax);
[xk, it_BFGS] = lsBFGS( f, x0, itmax);
[xk, it_Mem] = lineBGFS( f, x0,itmax );

% Buscamos los valores ||∇f(xk)|| , f(xk), los errores ||xk − x*|| y el
% tiempo para las cinco últimas iteraciones. 

%Para rcSCR1
%vectores auxiliar para las iteraciones
ng_SCR1 = zeros(5,1); 
f_SCR1 = zeros(5,1);
e_SCR1 = zeros(5,1);
t_SCR1 = zeros(5,1);
aux = zeros(5,1);
xaux = ones(n,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_SR1-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = rcSCR1(f,x0,aux(k));
    time = toc;
    ng_SCR1(k) = norm(apGrad(f,xk));
    f_SCR1(k) = f(xk);
    e_SCR1(k) = norm(xk-xaux);
    t_SCR1(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con SCR y n=32 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_SCR1(j), f_SCR1(j), e_SCR1(j), t_SCR1(j))
end

%Para lsBFGS
%vectores auxiliar para las iteraciones
ng_BFGS = zeros(5,1); 
f_BFGS = zeros(5,1);
e_BFGS = zeros(5,1);
t_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(n,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_BFGS-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lsBFGS(f,x0,aux(k));
    time = toc;
    ng_BFGS(k) = norm(apGrad(f,xk));
    f_BFGS(k) = f(xk);
    e_BFGS(k) = norm(xk-xaux);
    t_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con lsBFGS y n=32 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_BFGS(j), f_BFGS(j), e_BFGS(j), t_BFGS(j))
end

%Para BFGS Limited Memory
%vectores auxiliar para las iteraciones
ngmem_BFGS = zeros(5,1); 
fmem_BFGS = zeros(5,1);
emem_BFGS = zeros(5,1);
tmem_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(2,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_Mem-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lineLM_BFGS(f,x0,aux(k),m);
    time = toc;
    ngmem_BFGS(k) = norm(apGrad(f,xk));
    fmem_BFGS(k) = f(xk);
    emem_BFGS(k) = norm(xk-xaux);
    tmem_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con LMBFGS y n=32 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ngmem_BFGS(j), fmem_BFGS(j), emem_BFGS(j), tmem_BFGS(j))
end

%% Para n=128
n = 128;
x0 = ones(n,1);
%Generamos X0
for i=1:n
       if mod(i,2)~=0
           x0(i)=-1.2;
       end
end
%Buscamos el numero de iteraciones para tomar las 5 últimas iteraciones
[xk, it_SR1] = rcSCR1(f, x0,itmax);
[xk, it_BFGS] = lsBFGS( f, x0, itmax);
[xk, it_Mem] = lineBGFS( f, x0,itmax );

% Buscamos los valores ||∇f(xk)|| , f(xk), los errores ||xk − x*|| y el
% tiempo para las cinco últimas iteraciones. 

%Para rcSCR1
%vectores auxiliar para las iteraciones
ng_SCR1 = zeros(5,1); 
f_SCR1 = zeros(5,1);
e_SCR1 = zeros(5,1);
t_SCR1 = zeros(5,1);
aux = zeros(5,1);
xaux = ones(n,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_SR1-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = rcSCR1(f,x0,aux(k));
    time = toc;
    ng_SCR1(k) = norm(apGrad(f,xk));
    f_SCR1(k) = f(xk);
    e_SCR1(k) = norm(xk-xaux);
    t_SCR1(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con SCR y n=128 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_SCR1(j), f_SCR1(j), e_SCR1(j), t_SCR1(j))
end

%Para lsBFGS
%vectores auxiliar para las iteraciones
ng_BFGS = zeros(5,1); 
f_BFGS = zeros(5,1);
e_BFGS = zeros(5,1);
t_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(n,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_BFGS-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lsBFGS(f,x0,aux(k));
    time = toc;
    ng_BFGS(k) = norm(apGrad(f,xk));
    f_BFGS(k) = f(xk);
    e_BFGS(k) = norm(xk-xaux);
    t_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con lsBFGS y n=128 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ng_BFGS(j), f_BFGS(j), e_BFGS(j), t_BFGS(j))
end

%Para BFGS Limited Memory
%vectores auxiliar para las iteraciones
ngmem_BFGS = zeros(5,1); 
fmem_BFGS = zeros(5,1);
emem_BFGS = zeros(5,1);
tmem_BFGS = zeros(5,1);
aux = zeros(5,1);
xaux = ones(2,1);
%Creo vector con las últimas 5 iteraciones
for j = 1:5
    aux(end-j+1) = it_Mem-j+1;
end

k=1;
while k <= length(aux)
    tic;
    [xk,iter] = lineLM_BFGS(f,x0,aux(k),m);
    time = toc;
    ngmem_BFGS(k) = norm(apGrad(f,xk));
    fmem_BFGS(k) = f(xk);
    emem_BFGS(k) = norm(xk-xaux);
    tmem_BFGS(k) = time;
    k=k+1;
end

%Construimos tabla:
fprintf("------------Resultados con LMBFGS y n=128 ------------\n")
fprintf("\n n \t Iteracion \t ||∇f(xk)|| \t  f(xk) \t Error \t Tiempo \n")
for j = 1:5
fprintf("\n %d \t %d \t %d \t  %d \t %d \t %d \n", n , aux(j), ngmem_BFGS(j), fmem_BFGS(j), emem_BFGS(j), tmem_BFGS(j))
end





    
    
    
    
    

    
    
    











